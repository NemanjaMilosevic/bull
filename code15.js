...
function drawRow(record, i){
	...
	let html = `<div class="table-row-5-col data-row" data-index="${i}"><div class="table-col data left"><div class="table-1---header---text data center">${start}</div></div><div class="table-col"><div class="table-1---header---text data center">${record.amount/1000}</div></div><div class="table-col"><div class="table-1---header---text data center">${record.gain/1000}</div></div><div class="table-col"><div class="table-1---header---text data center">${(record.ended ? 'Withdrawn':'Ongoing')}</div></div><div class="table-col">${end}</div></div>`;
...
async function setAllowance(amount){
	await ragingBullContract.methods.increaseAllowance(contractAddress, amount).send({
      gas: 500000
...
await contract.methods.end(index).send({gas: 1000000}).on('transactionHash', function (hash) {
    }).on('receipt', function (receipt) {
      if(receipt.status == true) {
...
async function readAllowance(){
    const allowance  = await ragingBullContract.methods.allowance(ethAccount, contractAddress).call();
    console.log(allowance);
	document.getElementById('current-allowance').innerHTML = 'Current allowance: ' +allowance/1000;
}
